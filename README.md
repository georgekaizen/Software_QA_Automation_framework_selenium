...
├── reports/                    # Test reports generated by CI/CD pipeline
...
This framework includes a complete Jenkins pipeline configuration with quality gates. The pipeline automatically triggers on every GitHub push.
...
│   │                         PIPELINE STAGES                             │   │
│   ├─────────────────────────────────────────────────────────────────────┤   │
│   │  1. Checkout ──> 2. Setup Env ──> 3. Code Linting ──> 4. Security   │   │
│   │                                       (flake8,         (bandit,     │   │
│   │                                        pylint)          safety)     │   │
│   │                                                                     │   │
│   │  5. Run Tests ──> 6. Analyze Results ──> 7. Archive Artifacts       │   │
│   │     (pytest,         (Quality Gate:        (reports,                │   │
│   │      headless)        ≥80% pass rate)       screenshots)            │   │
│   └─────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
|------|------|-----------|----------|
| **Code Linting** | flake8, pylint | Reports generated | No |
| **Security Scan** | bandit, safety | Vulnerabilities reported | No |
| **Test Pass Rate** | pytest + JUnit | Must be ≥ 80% | **Yes** |

### Prerequisites for Jenkins

1. **Jenkins Server** (version 2.387+)
2. **Python 3.9+** installed on Jenkins agent
3. **Google Chrome** (or Firefox/Edge) installed on Jenkins agent
4. **Required Jenkins Plugins:**
   ...
   - JUnit Plugin
   - Warnings Next Generation Plugin (optional)
...
2. Search and install the following:
   ...
   - `HTML Publisher`
   - `JUnit`
   - `Warnings Next Generation` (optional, for linting reports)
...
> - Use a reverse proxy (nginx, ngrok)
> - Configure your firewall to allow GitHub's IP ranges
> - Use GitHub Actions instead if webhooks aren't possible

### Step 4: Create Jenkins Pipeline Job

1. Go to **New Item** in Jenkins
2. Enter name: `Automation_Framework_2026`
3. Select **Pipeline** and click OK
4. Configure the job:

   **General:**
   - ✓ GitHub project
   - Project url: `https://github.com/your-username/Automation_Framework_2026/`
   ...
   **Pipeline:**
   ...
   - SCM: `Git`
   - Repository URL: `https://github.com/your-username/Automation_Framework_2026.git`
   - Credentials: Select `github-credentials` (from Step 2)
...
1. **Manual Test:** Click **Build Now** to verify the pipeline works
2. **Webhook Test:** Make a commit and push to GitHub
3. Check the **Console Output** for any errors

### Pipeline Output

After each build, you'll have access to:

| Artifact | Description |
|----------|-------------|
| `junit-report.xml` | JUnit test results (parsed by Jenkins) |
| `test-report.html` | Visual HTML test report |
| `flake8-report.txt` | PEP8 linting results |
...
The pipeline runs tests in headless mode (no GUI) using the `--headless` flag:

```bash
# CI/CD command (used in Jenkinsfile)
pytest tests/ --headless -v

# Local headless testing
python3 -m pytest tests/ --headless -v
```

### Customizing Quality Gate Thresholds

Edit the `Jenkinsfile` to adjust the test pass rate threshold:

```groovy
environment {
    // Change this value to adjust the quality gate
    TEST_PASS_THRESHOLD = 80  // Minimum pass rate percentage
}
```
...
- Install Chrome/Firefox on the Jenkins agent
- For Docker agents, use a Selenium-ready image
...
- Ensure Jenkins user has write access to the workspace
- Check file permissions on the drivers/ directory

### Alternative: Jenkinsfile for Docker Agent

If you're using Docker agents, create a `Jenkinsfile.docker`:
